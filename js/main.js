...
jQuery(document).ready(function() {
	...
	jQuery("#mainmenu li").on('click', function(){
		...
		if ($(this).data('to') == 'mnav'){
			var ind = $(this).data('target') - 1;
			$('#js-mnav li:eq('+ ind +') a').trigger('click');
		}
	...
	renderCS($('#js-caoch'), data_caoch);
	renderCS($('#js-store'), data_store);


	//拍出热门活动
	function renderHotActive(dom, data){
		var html = [];
	//	html.push('<div class="owl-wrapper-outer">');
		for(var i = 0; i < data.length; i++){
			var dt = data[i];
			html.push('<div class="owl-carousel-item">');
	        html.push('    <a href=""><img src="'+ dt.img +'" alt="Owl Image"></a>');
	        html.push('    <h3><a href="'+ dt.href +'">'+ dt.tit +'</a></h3>');
	        html.push('    <div class="classes-description">');
	        html.push('        <p class="with-icon date">'+ dt.date +'</p>');
	        html.push('        <p class="with-icon time">'+ dt.time +'</p>');
	        html.push('        <p class="with-icon person"><a href="'+ dt.href +'">'+ dt.person +'</a></p>');
	        html.push('    </div>');
	        html.push('</div>');	
		}
	//	html.push('</div>');	
...
//	renderHotActive($('#js-hot-active'), data_active);
